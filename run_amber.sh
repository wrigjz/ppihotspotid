...
$mpirun -n `wc -l < $PBS_NODEFILE` -machinefile $PBS_NODEFILE \
    $AMBERHOME/bin/sander.MPI -O -i $scripts/sander.in -p an.top -c an.crd -ref an.crd \
    -o an_mini.out -r an_mini.res
#$AMBERHOME/bin/sander -O -i $scripts/sander.in -p an.top -c an.crd -ref an.crd \
#       -o an_mini.out -r an_mini.res
...
$AMBERHOME/bin/MMPBSA.py -O -i $scripts/mm_pbsa.in -o FINAL_RESULTS_MMPBSA_wt.dat \
          -do FINAL_DECOMP_MMPBSA_wt.dat -cp an_mini.top -y an_mini.crd
